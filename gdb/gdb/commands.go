...
func (c Command) Serialize(sequence string) *bytes.Buffer {
	var buffer = bytes.NewBufferString("")
	buffer.WriteString(sequence)
	if !c.IsCliCommand {
		buffer.WriteString("-")
	}
	buffer.WriteString(c.Operation)
...
func (gdb *Gdb) Send(operation string, arguments ...string) (map[string]interface{}, error) {
	return gdb.Send1([]Command{{Operation: operation, Arguments: arguments}})
}

func (gdb *Gdb) SendCLI(operation string, arguments ...string) (map[string]interface{}, error) {
	return gdb.Send1([]Command{{Operation: operation, Arguments: arguments, IsCliCommand: true}})
}

func (gdb *Gdb) SendDebug1(operation string, argument1 string) (map[string]interface{}, error) {
	return gdb.Send(operation, argument1)
}
func (gdb *Gdb) SendDebug2(operation, argument1, argument2 string) (map[string]interface{}, error) {
	return gdb.Send(operation, argument1, argument2)
}
func (gdb *Gdb) SendDebug3(operation, argument1, argument2, argument3 string) (map[string]interface{}, error) {
	return gdb.Send(operation, argument1, argument2, argument3)
}
func (gdb *Gdb) SendDebug4(operation, argument1, argument2, argument3, argument4 string) (map[string]interface{}, error) {
	return gdb.Send(operation, argument1, argument2, argument3, argument4)
}

func (gdb *Gdb) Send1(commands []Command) (map[string]interface{}, error) {
	log.Println("Send1", commands)
	...
	for idx, cmd := range commands {
		...
		if idx == 0 {
			buffer = cmd.Serialize(sequence)
		} else {
			buffer = cmd.Serialize("")
		}
...
func (gdb *Gdb) recordReader() {
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "=breakpoint-modified,bkpt=") && strings.Contains(line, "},{") {
			line = strings.ReplaceAll(line, ",bkpt=", ",bkpt=[") + "]"
		}
